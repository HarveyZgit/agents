...
`tier: core` 的片段由 `rules-sync` skill 装进本机各宿主：它从本仓库跟踪的分支拉片段到一个中立目录，再按宿主能力接线——能跟随活引用的宿主拿到指向该目录的引用，什么都不展开的宿主拿到剥掉 frontmatter 的拼装块，全局规则只存在于界面输入框的宿主拿到待粘贴文本。宿主清单和每个宿主为什么是这个模式，见该 skill 脚本里的 `HOSTS` 表；宿主路径只允许出现在那一个文件里。
